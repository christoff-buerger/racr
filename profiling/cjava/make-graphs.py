...
def plot3d(data):
    ...
    for l_value in set(data[l]):
        ...
        y_values = []
        z_values = []
        for y_value in set(map(lambda row: row[y], l_points)):
            ...
            line = ax.plot(x_line, y_line, z_line, '-' + markers[0], color=colors[0])
        ...
        ax.plot_surface(x_values, y_values, z_values, alpha=0.3, color=colors[0])
        lines.append(line[0])
