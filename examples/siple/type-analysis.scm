     ...
     (with-specification
      ...
      (ag-rule
        ...
        (lambda (n)
          (let ((l-type (att-value 'type (ast-child 1 n))))
            (if (type-pointer? l-type)
                (type-eq? (type-rtype l-type) (att-value 'type (ast-child 2 n)))
                (type-error-type)))))
            ...
            (if procedure-decl
                (let ((procedure-rtype (type-rtype (att-value 'type procedure-decl))))
                  (if (> (ast-num-children (ast-child 1 n)) 0)
                      (type-eq? (att-value 'type (ast-child 1 (ast-child 1 n))) procedure-rtype)
       ...
       (Not
        0
        (lambda (n)
          (if (type-boolean? (att-value 'type (ast-child 1 n)))
              (type-boolean)
              (type-error-type))))
       ...
       (RealCoercion
        0
        (lambda (n)
          (if (type-integer? (att-value 'type (ast-child 1 n)))
              (type-real)
              (type-error-type))))
       ...
       (LogicExpression
        0
        (lambda (n)
          (if (and (type-boolean? (att-value 'type (ast-child 1 n))) (type-boolean? (att-value 'type (ast-child 2 n))))
              (type-boolean)
              (type-error-type))))
       ...
       (Equal
        0
        (lambda (n)
          (if (type-beq? (att-value 'type (ast-child 1 n)) (att-value 'type (ast-child 2 n)))
              (type-boolean)
              (type-error-type))))
